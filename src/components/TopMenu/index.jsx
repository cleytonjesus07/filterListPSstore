...
export default function TopMenu({ setActive }) {
    ...
    return (
        <div className="w-full max-md:flex-col-reverse max-md:items-center flex justify-between px-10 pt-10 pb-2 bg-black bg-opacity-40 select-none">
            <div className="space-x-10 flex items-end">
                ...
                <button onClick={() => setActive(true)} className="flex  items-center opacity-80 hover:opacity-100 transition-all ease-in-out"><AiOutlineSearch className="w-5 h-auto mr-2" />Search</button>
                <button onClick={() => setIsOpenCart(true)} className="flex items-center opacity-80 hover:opacity-100 transition-all ease-in-out"><BsCart4 className="mr-2" />Cart <span className="ml-2 text-xs bg-white bg-opacity-30 px-1 ">{cartLength}</span></button>
            </div>
